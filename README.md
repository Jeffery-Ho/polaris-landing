...
- The compact header keeps Install as the primary product action and provides a persisted light/dark theme switch. Product actions use the shared gradient label/border treatment with solid interiors; the hero video uses a crisp bordered surface, the failure-recovery play control is minimal, and the PayPal support link uses a high-contrast orange CTA.
